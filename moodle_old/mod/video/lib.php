...
function video_add_instance($data, $mform = null) {
    ...
    if ($mform) {
        $data->content       = $data->video['text'];
        $data->contentformat = $data->video['format'];
    ...
    if ($mform and !empty($data->video['itemid'])) {
        $draftitemid = $data->video['itemid'];
        $data->content = file_save_draft_area_files($draftitemid, $context->id, 'mod_video', 'content', 0, video_get_editor_options($context), $data->content);
...
function video_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    ...
    if ($arg == 'index.html' || $arg == 'index.htm') {
        ...
        // remove @@PLUGINFILE@@/
        $content = str_replace('@@PLUGINFILE@@/', '', $video->content);
        ...
        if (!$file = $fs->get_file_by_hash(sha1($fullpath)) or $file->is_directory()) {
            $video = $DB->get_record('video', array('id'=>$cm->instance), 'id, legacyfiles', MUST_EXIST);
            if ($video->legacyfiles != RESOURCELIB_LEGACYFILES_ACTIVE) {
                return false;
            }
            if (!$file = resourcelib_try_file_migration('/'.$relativepath, $cm->id, $cm->course, 'mod_video', 'content', 0)) {
                return false;
            }
            //file migrate - update flag
            $video->legacyfileslast = time();
