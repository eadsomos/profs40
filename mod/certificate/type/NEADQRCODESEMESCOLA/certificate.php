...
$papel = "SELECT r.shortname FROM mdl_role_assignments 
AS ra LEFT JOIN mdl_user_enrolments 
AS ue ON ra.userid = ue.userid LEFT 
JOIN mdl_role AS r ON ra.roleid = r.id 
LEFT JOIN mdl_context AS c ON c.id = ra.contextid 
LEFT JOIN mdl_enrol AS e ON e.courseid = c.instanceid 
AND ue.enrolid = e.id WHERE ue.userid = " . $USER->id . " AND e.courseid = ".$PAGE->course->id."";
...
if ($papelcursista = $DB->get_record_sql($papel)) {
$papelcursista = $papelcursista->shortname;
} else {
$value = '';
}
...
if ($papelcursista == "tutor"){
	$strtexto =  "A Somos Educação certifica que <b>{$strname}</b>, portador do CPF {$cpfcursista}, concluiu como tutor o curso <b>{$strcurso}</b> realizado de <b>{$comecoCurso}</b> a <b>{$strdata}</b>, com carga horária de {$strhoras}.";
}else{
	$strtexto =  "A Somos Educação certifica que <b>{$strname}</b>, portador do CPF {$cpfcursista}, concluiu o curso <b>{$strcurso}</b> realizado de <b>{$comecoCurso}</b> a <b>{$strdata}</b>, com carga horária de {$strhoras}.";
}
